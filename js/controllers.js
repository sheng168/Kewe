...
angular.module('starter.controllers', ['firebase', 'UserService']).controller('AppCtrl', function ($scope) {
    ...
    $scope.saveProfile = function () {
        ...
        if ($scope.user.phone)
            parse.set('username', $scope.user.phone);
        if ($scope.user.email)
            parse.set('email', $scope.user.email);
